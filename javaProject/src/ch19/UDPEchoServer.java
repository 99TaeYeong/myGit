...
public class UDPEchoServer {
	public UDPEchoServer(int port) {
		try {
			...
			while(true) {
				//udp 방식은 데이터를 바이트 배열로 전송함, String을 바로보낼 수 없음
				...
				InetAddress ia = dp.getAddress();//클라이언트측 ip주소를 get 
				port = dp.getPort(); //클라이언트측 port번호를 get
				System.out.println("클라이언트 아이피: "+ia+"클라이언트 포트: "+port);
